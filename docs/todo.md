...
| 16 | Custom boot & shutdown UX | Ops | Customize the boot and shutdown experience so the device feels like an appliance, not a Linux box: quiet/branded boot (suppress kernel + console text, Plymouth splash or boot logo), ideally a splash held until the dashboard is up, and a clean branded shutdown/power-off sequence. | P3 | Done | Implemented via `scripts/boot-branding/` (idempotent `install.sh`): Plymouth `baja` theme (emblem on boot, GOODBYE skull on shutdown), quiet console (`console=serial` + `quiet vt.global_cursor_default=0 logo.nologo`), U-Boot logo staged, Chromium `splash.html`→dashboard handoff, cursor hidden via the `cursor-hide` pm2 app (`unclutter`). Known limits: HDMI is dark until the kernel DRM so the U-Boot logo can't display over HDMI on this board; a brief desktop/mode-set flash remains during the X handoff. |

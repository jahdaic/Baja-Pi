...
const ReactLinearGauge: React.FC<ILinearGauge> = ({ height = '100%', width = '100%', value, ...props }) => {
	...
	useEffect(() => {
		if (gauge && canvas.current)
			gauge.value = Math.min(value || 0, gauge.options.maxValue || 0, props.maxValue || 0) || 0;
	}, [value]);

	useEffect(() => {
		const newHeight = height.toString().includes('%')
			? (canvas.current?.parentElement?.clientHeight || 0) *
				(Number(height.toString().substring(0, height.toString().length - 1)) / 100)
			: canvas.current?.parentElement?.clientHeight;
		const newWidth = width.toString().includes('%')
			? (canvas.current?.parentElement?.clientWidth || 0) *
				(Number(width.toString().substring(0, width.toString().length - 1)) / 100)

...
| 7 | pm2 ecosystem config | Repo / Ops | Root `ecosystem.config.cjs` has `gpsd` + `gps-server` (node HTTP bridge on `:8000`), both `pm2 save`d for boot. Still to add: `ui-vite` (or a static server for `dist/`) and `chromium-kiosk`. | P2 | In Progress | Model on Alice's `~/Companion/ecosystem.config.cjs`. |
